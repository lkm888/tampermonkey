// ==UserScript==
// @name         GDS 账户信息增强版 (v3.2.87-mod4 - 布局优化与功能增强)
// @namespace    http://tampermonkey.net/
// @version      3.2.87.4
// @description  [v3.2.87-mod4]: 1.使用Flexbox重构UI布局，更灵活现代。2.新增日志面板折叠/展开功能。3.将“余额”列加入显示控制。4.首次加载时默认显示“账号”和“余额”列。
...
(async function() {
  ...
  // --- 常量定义 ---
  const SCRIPT_VERSION = 'v3.2.87.4';
  const KEYS = { RELOAD_DELAY: 'gds_pending_reload_delay_after_401_v3.2', ACCOUNT_CACHE: 'gds_account_data_cache_idb_v3.2', ACCOUNT_ORDER: 'gds_account_order_idb_v3.2', THEME_PREF: 'gds_theme_preference_v3.1.7', COLUMN_VIS: 'gds_column_visibility_v3.1.8', SORT_CONF: 'gds_sort_config_v3.1.8', LAST_REFRESH: 'gds_last_successful_refresh_v3.1.8.1', LOGS_VISIBLE: 'gds_logs_visibility_v3.2.87.4' };
  const API_BASE_URL = 'https://admin.gdspay.xyz/api';
  const API_PATHS = { REFRESH_TOKEN: '/auth/v1/refresh', ACCOUNT_VIEW: '/tripartite/v1/account/view', STATUS_MODIFY: '/tripartite/v1/account/status/modify', MANUAL_TRANSFER: '/tripartite/v1/transfer/manual' };
  ...
  let areLogsVisible = true;

  // --- [优化] CSS样式，使用Flexbox布局 ---
  document.head.appendChild(Object.assign(document.createElement('style'), { innerHTML: `
    ...
    body { background-color: var(--body-bg); color: var(--text-color); transition: background-color 0.3s, color 0.3s; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; }
    ...
    #gds-control-panel { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; font-size: 12px; padding: 6px; background-color: var(--panel-bg); border: 1px solid var(--panel-border); border-radius: 4px; }
    ...
    #gds-content-wrapper { flex-grow: 1; overflow: auto; /* [新增] 关键属性，让表格区可滚动 */ border: 1px solid var(--panel-border); border-radius: 4px; }
    ...
    #gds-account-info input[type="text"].remarks-input { box-sizing: border-box; padding: 2px 4px; border: 1px solid var(--input-border); background-color: var(--input-bg); color: var(--input-text); width: 100%; min-width: 80px; }
    .col-statusOp { text-align: center; min-width: 170px; white-space: normal;}
    #gds-account-info input[type="number"].autostopreceipt-setting, #gds-account-info input[type="number"].autotransfer-setting { width: 80px; text-align: right; }
    #gds-account-info select.autotransfer-setting { width: 100%; max-width: 120px; }
    ...
    .status-op-btn:hover { background-color: var(--button-hover-bg); }
    ...
    .log-status-change { color: blue; } .log-api-op-success { color: green; }
    .log-api-op-fail { color: red; } .log-transfer-success { color: #008000; font-weight: bold; } .log-transfer-fail { color: #B22222; font-weight: bold; }
  ...
  // --- [优化] HTML 结构, 使用 Flexbox 布局 ---
  document.body.insertAdjacentHTML('beforeend', `
    <div id="gds-enhanced-ui">
        <div id="gds-header">
            <div id="gds-control-panel">
              <span>搜索:</span> <input id="gds-search" placeholder="ID/平台/账号/手机/备注/失败原因/日志" title="可搜索多个关键词，用空格隔开"/>
  ...
  const dbHelper = (() => {
    ...
    const trimStore = async (store, max) => new Promise(async (resolve, reject) => {
      ...
      countReq.onsuccess = () => { if (countReq.result > max) { let numToDelete = countReq.result - max; const cursorReq = os.openCursor(null, 'next'); cursorReq.onsuccess = e => { const cursor = e.target.result; if (cursor && numToDelete > 0) { os.delete(cursor.primaryKey); numToDelete--; cursor.continue(); }}; }};
    });
    const clear = async store => new Promise(async (resolve, reject) => { (await getObjectStore(store, 'readwrite')).clear().transaction.oncomplete = () => resolve(); });
    const deleteDB = async () => { if (dbPromise) { (await dbPromise).close(); dbPromise = null; }
      return new Promise((res, rej) => { const req = indexedDB.deleteDatabase(DB_NAME); req.onsuccess = () => (console.log(`IndexedDB ${DB_NAME} 已删除。`), res()); req.onerror = e => (console.error(`删除数据库 ${DB_NAME} 失败:`, e), rej(e)); req.onblocked = () => (alert(`无法删除数据库 ${DB_NAME}，请关闭使用此脚本的其他标签页。`), rej('数据库删除被阻塞')); });
    };
  ...
  const columnConfig = [
    { id: 'deleteAction', label: '删', sortable: false, hideable: false, cssClass: 'col-delete' },
    { id: 'id', label: 'ID', sortable: true, hideable: false, dataKey: 'id', cssClass: 'col-id' },
  ...
  const copyToClipboard = (txt, e) => navigator.clipboard.writeText(txt).then(() => showToast(`已复制: ${txt.length > 30 ? txt.substring(0,27)+'...' : txt}`, e.clientX + 10, e.clientY + 10)).catch(() => showToast('复制失败', e.clientX + 10, e.clientY + 10));
  ...
  const fmtInt = s => { if (isNaN(s) || s < 0) return 'N/A'; const m = Math.floor(s / 60), sec = s % 60; return `${m > 0 ? `${m}m ` : ''}${sec > 0 || m === 0 ? `${sec}s` : ''}`.trim(); };
  ...
  async function _addLogEntry(logArr, dbStore, maxMem, maxDb, entryData) {
    ...
    try { await dbHelper.addLog(dbStore, newLog); await dbHelper.trimStore(dbStore, maxDb); } catch (e) { console.error(`保存日志到 DB (${dbStore}) 失败:`, e); }
    _renderLogs();
  }
  function _renderLogs() {
      const render = (container, logArr, title) => {
      ...
      render(logDisplayContainer, operationLogs, '操作与变动日志');
      render(frozenLogDisplayContainer, frozenBalanceIncreaseLogs, '冻结金额增加日志');
  }
  ...
  const loadSetting = async (key, defaultVal) => { try { const s = await dbHelper.get(dbHelper.STORES.SETTINGS, key); return s ? s.value : defaultVal; } catch (e) { console.error(`加载设置 ${key} 失败:`, e); return defaultVal; } };
  const saveSetting = async (key, value) => { try { await dbHelper.set(dbHelper.STORES.SETTINGS, { key, value }); } catch (e) { console.error(`保存设置 ${key} 失败:`, e); } };
  ...
  function parseGmHeaders(headerStr) {
      ...
      headerStr.split('\r\n').forEach(headerPair => { const index = headerPair.indexOf(': '); if (index > 0) { headers.append(headerPair.substring(0, index), headerPair.substring(index + 2)); } });
      return headers;
  }
  const gmRequest = details => new Promise((res, rej) => {
    GM_xmlhttpRequest({
      ...
      onerror: r => rej(new Error(`网络错误: ${r.error || r.statusText || '未知错误'}`)),
  ...
  async function refreshAuthTokens() {
      ...
      refreshPromise = (async () => {
          ...
          if (!refreshToken) { console.error("刷新Token缺失"); showFetchStatus('刷新Token缺失，请重新登录。', 'error', 0); return false; }
          showFetchStatus('Token已过期，正在刷新...', 'info', 0);
          try {
              const res = await gmRequest({ method: "GET", url: API_BASE_URL + API_PATHS.REFRESH_TOKEN, headers: { "authorization": refreshToken }, responseType: "json" });
              if (res.ok && res.rawJson?.code === 1) {
                  const { token: newAccessToken, refreshToken: newRefreshToken } = res.rawJson.data;
                  localStorage.setItem('token', newAccessToken); localStorage.setItem('refreshToken', newRefreshToken); token = newAccessToken; refreshToken = newRefreshToken; showFetchStatus('Token刷新成功!', 'success', 2000); return true;
              } else { const errMsg = res.rawJson?.msg || `状态码: ${res.status}`; showFetchStatus(`刷新Token失败: ${errMsg}`, 'error', 4000); return false; }
          } catch (e) { showFetchStatus(`刷新Token请求异常: ${e.message}`, 'error', 4000); return false; }
  ...
  async function apiRequest(details, retryCount = 0) {
      ...
      try {
          ...
          if (error.message !== 'Token刷新失败，无法重试原请求。') showFetchStatus(`API请求异常: ${error.message}`, 'error', 4000);
    ...
    try {
      const res = await apiRequest({ method: "POST", url: API_BASE_URL + API_PATHS.STATUS_MODIFY, headers: { "content-type": "application/json" }, data: JSON.stringify({ accountId: parseInt(accId), accountName, tripartiteId, accountStatus: newStatus }), responseType: "json" });
      const r = res.rawJson;
      if (res.ok && r?.code === 1) {
      ...
      } else { const errMsg = r ? r.msg : (res.rawText || '未知错误'); showFetchStatus(`ID ${accId} (${srcAction}): 状态设置失败 - ${errMsg}`, 'error', 4000); _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName, message: `${srcAction}: 状态设置 <span class="log-api-op-fail">(失败: ${esc(errMsg)})</span>` }); return false; }
    } catch (e) { console.error(`${srcAction} - 状态设置API错误 (ID ${accId}):`, e); showFetchStatus(`ID ${accId} (${srcAction}): 状态设置请求异常`, 'error', 4000); _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName, message: `${srcAction}: 状态设置 <span class="log-api-op-fail">(请求异常)</span>` }); return false; }
  }

  // 自动化任务 (无大变化)
  async function checkAndPerformAutoStopReceipt() { /* ... */ }
  async function checkAndPerformAutoReEnable() { /* ... */ }
  async function checkAndPerformAutoTransfers() { /* ... */ }
  // (为简洁起见，这部分自动化逻辑函数保持不变，它们与UI重构无关)
    ...
    for (const accId of accs) {
      ...
      if (acc.balance > trigAmt && acc.apiStatus === API_STATUS.ENABLED) {
        ...
        _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-attempt">自动止收触发: 余额 ${fmtAmt(acc.balance)} > ${fmtAmt(trigAmt)}.</span>` });
  ...
  async function checkAndPerformAutoReEnable(specificAccId = null) {
    ...
    for (const accId of accs) {
      ...
      if (acc.apiStatus === API_STATUS.STOP_RECEIPT && c.isAutoStoppedByScript && acc.balance < trigAmt) {
        ...
        _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-autorenable-attempt">自动解除止收触发: 余额 ${fmtAmt(acc.balance)} < ${fmtAmt(trigAmt)}.</span>` });
...
async function checkAndPerformAutoTransfers(specificAccId = null) {
  ...
  for (const accId of specificAccId ? [specificAccId] : Object.keys(accountDataCache)) {
    ...
    if (!s.payeeId || !s.transferMode) { _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-fail">自动划转配置不完整</span>` }); continue; }

    const txPerc = parseFloat(s.transferPercentage);
    if (isNaN(txPerc) || txPerc <= 0 || txPerc > 1) { _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-fail">自动划转失败: 无效的划转比例</span>` }); continue; }

    const baseAmount = acc.balance * txPerc;
    let finalTransferAmount = Math.round(baseAmount * (Math.random() * (RANDOM_TRANSFER_MAX_FACTOR - RANDOM_TRANSFER_MIN_FACTOR) + RANDOM_TRANSFER_MIN_FACTOR));
    if (s.roundToInteger) finalTransferAmount = Math.floor(finalTransferAmount / 100) * 100;
    const amtInCents = Math.floor(finalTransferAmount * 100);
    if (amtInCents <= 0) continue;
    ...
    const commonLogMsg = `自动划转 ${fmtAmt(amtInCents / 100)} 到 ${esc(payeeName)}`;

    c.lastTransferAttemptTime = nowPerAcc; await saveAccountDataToDB();
    ...
    try {
      const res = await apiRequest({ method: "POST", url: API_BASE_URL + API_PATHS.MANUAL_TRANSFER, headers: { "content-type": "application/json" }, data: JSON.stringify(payload), responseType: "json" });
      if (res.ok && res.rawJson?.code === 1) { c.lastSuccessfulTransferTime = Date.now(); _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-success">${commonLogMsg} 成功!</span>` }); }
      else { c.lastFailedTransferTime = Date.now(); const errMsg = res.rawJson ? res.rawJson.msg : (res.rawText || '未知错误'); _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-fail">${commonLogMsg} 失败: ${esc(errMsg)}</span>` }); }
    } catch (e) { c.lastFailedTransferTime = Date.now(); _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: acc.accountName, message: `<span class="log-transfer-fail">${commonLogMsg} 请求异常</span>` });
    } finally { delete c.lastTransferAttemptTime; await saveAccountDataToDB(); }
  }
}

  function calculateEstimatedHourlyRate(accCache) {
      const tenMinAgoTs = Date.now() - (10 * 60 * 1000);
      const totalIncrease = Object.values(accCache).reduce((overallAcc, c) => {
          if (!c?.current || c.current.isDisappeared || !Array.isArray(c.current.balanceHistory) || c.current.balanceHistory.length < 2) return overallAcc;
          const recentHistory = c.current.balanceHistory.filter(h => h.timestamp >= tenMinAgoTs && typeof h.balance === 'number').sort((a, b) => a.timestamp - b.timestamp);
          if (recentHistory.length < 2) return overallAcc;
          return overallAcc + recentHistory.reduce((acc, currH, i, arr) => i === 0 ? acc : acc + (currH.balance > arr[i - 1].balance ? currH.balance - arr[i - 1].balance : 0), 0);
      }, 0);
      if (totalIncrease === 0) return `预计速度: <span class="rate-stagnant">N/A (近10分钟无增)</span>`;
      return `预计速度: <span class="rate-positive"><b>+${fmtAmt(totalIncrease * 6)}</b>/小时</span>`;
  }
  const getApiStatusOrder = s => ({ [API_STATUS.ENABLED]: 0, [API_STATUS.STOP_RECEIPT]: 1, [API_STATUS.DISAPPEARED]: 2, [API_STATUS.CUSTOM_STOP]: 3 }[s] ?? 99);
  ...
  function renderTable() {
    // ... 表格渲染核心逻辑 (无大变化, 但现在会检查 balance 的可见性) ...
    // ... 为简洁起见，此处省略，核心逻辑不变 ...
    ...
    sortedAccounts.sort((a, b) => {
      const orderA = getApiStatusOrder(a.current?.apiStatus); const orderB = getApiStatusOrder(b.current?.apiStatus); if (orderA !== orderB) return orderA - orderB;
      const currentSortCol = columnConfig.find(c => c.id === sortConfig.key);
      let valA, valB;
      if (currentSortCol) {
        if (['id', 'loginStatus'].includes(currentSortCol.id)) { valA = parseInt(a.current?.[currentSortCol.dataKey] ?? 0); valB = parseInt(b.current?.[currentSortCol.dataKey] ?? 0); }
        ...
        else { valA = String(a.current?.[currentSortCol.dataKey] ?? '').toLowerCase(); valB = String(b.current?.[currentSortCol.dataKey] ?? '').toLowerCase(); }
      } else { valA = parseInt(a.current?.id ?? 0); valB = parseInt(b.current?.id ?? 0); }
      ...
      return comparison === 0 ? (parseInt(a.current?.id ?? 0) - parseInt(b.current?.id ?? 0)) : comparison;
    });
    ...
    sortedAccounts.forEach(c => {
      ...
      columnConfig.forEach(col => {
        ...
        switch (col.id) {
          ...
          case 'id': content = esc(acc.id); break;
          case 'platform': content = esc(acc.platform); break;
          case 'accountName': content = esc(acc.accountName); break;
          case 'phone': content = esc(acc.phone); break;
          case 'balance':
            if (acc.balance >= 0) { let tierSuffix = '0'; if (acc.balance >= 150000) tierSuffix = '4'; else if (acc.balance >= 100000) tierSuffix = '3'; else if (acc.balance >= 50000) tierSuffix = '2'; else if (acc.balance >= 10000) tierSuffix = '1'; cellClass += ` balance-tier-${tierSuffix}`; }
          ...
          case 'failedReason': content = esc(acc.failedReason || '无'); break;
          case 'balanceFailed': content = acc.balanceFailed ? '是' : '否'; if (acc.balanceFailed) cellClass += ' balance-failed-yes'; break;
          case 'remarks': content = `<input type="text" class="remarks-input" data-account-id="${esc(acc.id)}" value="${esc(acc.remarks || '')}" placeholder="备注...">`; break;
    ...
    if (sortedAccounts.length === 0) tableContainer.querySelector('tbody').innerHTML = `<tr><td colspan="${columnConfig.length}" style="text-align: center; padding: 20px;">没有账户数据或搜索结果。</td></tr>`;
  ...
  async function loadPersistedData() {
    ...
    try {
        ...
        accountOrder = order || [];
        lastSuccessfulDataTimestamp = lastRefresh ? new Date(lastRefresh) : null;
        sortConfig = sortConf;
        areLogsVisible = logsVis;
        ...
        if (colVis === null) {
            ...
            columnConfig.forEach(c => {
                if (c.hideable) {
                    columnVisibility[c.id] = c.defaultVisible;
                } else {
                    columnVisibility[c.id] = true; // 不可隐藏的列总是可见
                }
            });
        ...
        for (const accId in accountDataCache) {
            const c = accountDataCache[accId];
            Object.assign(c, {
                autoTransferSettings: initAutoTxSettings(c.autoTransferSettings),
                autoStopReceiptSettings: initAutoStopSettings(c.autoStopReceiptSettings),
                current: c.current || {}
            });
  ...
  async function saveAccountDataToDB() {
    try { await Promise.all([ dbHelper.set(STORES.ACC_DATA, KEYS.ACCOUNT_CACHE, accountDataCache), dbHelper.set(STORES.ACC_ORDER, KEYS.ACCOUNT_ORDER, accountOrder) ]); }
    catch (e) { console.error("保存账户数据到DB失败:", e); }
  }
  ...
  async function fetchAccountData(isInitialLoad = false) {
    ...
    try {
        // [核心] 数据获取与处理逻辑
        // ... 此处逻辑与原版基本一致，仅做微小调整，为简洁省略 ...
        // ... 重点在于数据处理完毕后，调用渲染和自动化检查 ...
      const res = await apiRequest({ method: "GET", url: API_BASE_URL + API_PATHS.ACCOUNT_VIEW, responseType: "json" });
      if (!res.ok || res.rawJson?.code !== 1 || !Array.isArray(res.rawJson?.data?.list)) {
        ...
        showFetchStatus(`API错误: ${errorMsg}`, 'error', 7000);
        if (lastRefreshTimeEl) { lastRefreshTimeEl.innerText = `API错误于: ${fmtDT(fetchAttemptTime)}`; lastRefreshTimeEl.classList.add('error'); }
        if (Object.keys(accountDataCache).length === 0) tableContainer.innerHTML = `获取数据失败：${esc(errorMsg)}`; else renderTable(); return;
      }
      ...
      apiList.forEach(apiItem => {
          ...
          let c = accountDataCache[accIdStr] = accountDataCache[accIdStr] || { current: {}, autoTransferSettings: initAutoTxSettings(), autoStopReceiptSettings: initAutoStopSettings() };
          Object.assign(c, { autoTransferSettings: initAutoTxSettings(c.autoTransferSettings), autoStopReceiptSettings: initAutoStopSettings(c.autoStopReceiptSettings) });
          const prev = { ...c.current };
          const current = {
              id: accIdStr, platform: apiItem.tripartiteId, accountName: apiItem.accountName, phone: apiItem.otpReceiver,
              balance: parseFloat(apiItem.balance) / 100, frozenBalance: parseFloat(apiItem.frozenBalance) / 100,
              apiStatus: parseInt(apiItem.accountStatus), lastChangeTime: prev.lastChangeTime || nowFormattedStr,
              isDisappeared: false, balanceHistory: prev.balanceHistory ? [...prev.balanceHistory] : [],
              remarks: prev.remarks || '', loginStatus: parseInt(apiItem.loginStatus), failedReason: apiItem.failedReason, balanceFailed: apiItem.balanceFailed,
          };
          if (!isInitialLoad && prev.balance > 0 && current.balance === 0) current.balance = prev.balance;
          if (!isInitialLoad && prev.frozenBalance > 0 && current.frozenBalance === 0) current.frozenBalance = prev.frozenBalance;
          ...
          if (prev.frozenBalance !== undefined && current.frozenBalance !== prev.frozenBalance) { const diff = current.frozenBalance - prev.frozenBalance; logMsgParts.push(`冻结: ${fmtAmt(prev.frozenBalance)} → ${fmtAmt(current.frozenBalance)}`); sigAmtChanged = true; if (diff > 0 && !isInitialLoad) _addLogEntry(frozenBalanceIncreaseLogs, STORES.FROZEN_LOGS, MAX_FROZEN_LOG_MEM, MAX_FROZEN_LOG_DB, { accountId: accIdStr, accountName: current.accountName, message: `冻结增加: ${fmtAmt(prev.frozenBalance)} → ${fmtAmt(current.frozenBalance)}` }); }
          if (sigAmtChanged) current.lastChangeTime = nowFormattedStr;
          if (!isInitialLoad && logMsgParts.length > 0) _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accIdStr, accountName: current.accountName, message: logMsgParts.join('， ') });
      ...
      accountOrder.forEach(accIdStr => {
          if (!currentApiAccountIds.has(accIdStr) && accountDataCache[accIdStr]?.current && !accountDataCache[accIdStr].current.isDisappeared) {
              const c = accountDataCache[accIdStr]; c.current.isDisappeared = true; c.current.apiStatus = API_STATUS.DISAPPEARED; c.current.loginStatus = 0; c.current.failedReason ="";
              _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accIdStr, accountName: c.current.accountName, message: '<span class="status-disappeared">账号在API响应中消失</span>' });
          }
      });
      await saveAccountDataToDB();
      renderTable();
      hourlyRateDisplay.innerHTML = calculateEstimatedHourlyRate(accountDataCache);
      await checkAndPerformAutoTransfers();
      await checkAndPerformAutoStopReceipt();
      await checkAndPerformAutoReEnable();
    } catch (e) {
        ...
        console.error('FetchAccountData 异常:', e);
        if (lastRefreshTimeEl) { lastRefreshTimeEl.innerText = `脚本错误于: ${fmtDT(fetchAttemptTime)}`; lastRefreshTimeEl.classList.add('error'); }
        if (Object.keys(accountDataCache).length === 0) tableContainer.innerHTML = `获取数据时发生脚本错误: ${esc(e.message)}`; else renderTable();
    }
  }
  ...
  async function handleHeaderClick(e) {
    ...
    const col = columnConfig.find(c => c.id === th.dataset.colId); if (!col?.sortable) return;
    sortConfig = sortConfig.key === th.dataset.colId ? { key: th.dataset.colId, direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' } : { key: th.dataset.colId, direction: 'asc' };
  ...
  async function _handleSettingChange(e, type) {
    ...
    const settingName = t.dataset.setting;
    const newValue = (t.type === 'checkbox') ? t.checked : t.value;
    if (String(settingsObj[settingName]) === String(newValue)) return;
    settingsObj[settingName] = newValue;
    await saveAccountDataToDB();
    _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, accountName: cacheEntry.current?.accountName || 'N/A', message: `<span class="log-setting-change">${type === 'autoTransfer' ? '划转' : '止收'}设置 “${settingName}” 已更新</span>` });
    showToast(`ID ${accId}: 设置已保存`, e.clientX, e.clientY, 1000);
  }
  ...
  async function handleTableInteraction(e) {
    ...
    if (t.classList.contains('delete-account-btn')) {
      const accId = t.dataset.accountId;
      if (confirm(`确定要从本地删除账户 ID: ${accId} 的所有记录吗？`)) {
        delete accountDataCache[accId]; accountOrder = accountOrder.filter(id => id !== accId);
        await saveAccountDataToDB();
        _addLogEntry(operationLogs, STORES.OP_LOGS, MAX_LOG_MEM, MAX_LOG_DB, { accountId: accId, message: `<span class="log-local-delete">本地账户记录已删除</span>` });
        renderTable();
      }
      return;
    }
    ...
    if (t.classList.contains('status-op-btn') && t.dataset.op === 'set-status') {
      const accId = t.closest('tr').dataset.accountId; const newStatus = parseInt(t.dataset.status);
    ...
    // 各种设置输入框/选择框/复选框的即时保存
    const isSettingElement = t.classList.contains('autotransfer-setting') || t.classList.contains('autostopreceipt-setting');
    if (isSettingElement && (t.tagName === 'SELECT' || t.type === 'checkbox')) {
        const type = t.classList.contains('autotransfer-setting') ? 'autoTransfer' : 'autoStopReceipt';
        _handleSettingChange(e, type);
    }
  }

  async function handleTableBlur(e) {
      const t = e.target;
      if (t.tagName === 'SELECT' || t.type === 'checkbox') return; // 这些已由 change/click 事件处理

      if (t.classList.contains('remarks-input')) {
          const accId = t.dataset.accountId;
          if (accountDataCache[accId]) {
              accountDataCache[accId].current.remarks = t.value;
              await saveAccountDataToDB();
          }
      } else if (t.classList.contains('autotransfer-setting') || t.classList.contains('autostopreceipt-setting')) {
          const type = t.classList.contains('autotransfer-setting') ? 'autoTransfer' : 'autoStopReceipt';
          _handleSettingChange(e, type);
      }
  }
  ...
  async function initialize() {
      console.log(`GDS 账户信息增强版 (${SCRIPT_VERSION}) 启动...`);
      ...
      if (pendingReloadTimestamp > 0 && (Date.now() - pendingReloadTimestamp < RELOAD_FLAG_GRACE_MS)) {
          showFetchStatus(`检测到401后刷新，等待 ${RELOAD_DELAY_MS / 1000} 秒后加载...`, 'info', 0);
          await GM_setValue(KEYS.RELOAD_DELAY, 0);
          await new Promise(res => setTimeout(res, RELOAD_DELAY_MS));
      }

      // 绑定事件
      searchInput.addEventListener('input', () => { renderTable(); _renderLogs(); });
      ...
      tableContainer.addEventListener('click', handleTableInteraction);
      tableContainer.addEventListener('contextmenu', handleTableInteraction);
      tableContainer.addEventListener('change', handleTableInteraction); // 捕获 select 变化
      tableContainer.addEventListener('blur', handleTableBlur, true); // 捕获输入框失焦
      tableContainer.addEventListener('keydown', e => { if (e.key === 'Enter' && e.target.tagName === 'INPUT') e.target.blur(); });
      // ... 其他按钮事件
        D('gds-clear-log').addEventListener('click', async e => {
            if (confirm('确定要清空所有日志吗？')) {
                operationLogs = []; frozenBalanceIncreaseLogs = [];
                await Promise.all([dbHelper.clear(STORES.OP_LOGS), dbHelper.clear(STORES.FROZEN_LOGS)]);
                _renderLogs(); showToast('日志已清空', e.clientX, e.clientY);
            }
        });
        D('gds-clear-prev-data').addEventListener('click', async e => {
            if (confirm('警告：这将清空所有本地缓存和设置！确定要重置脚本吗？')) {
      ...
      if (token) {
          ...
          if (!refreshIntervalId) refreshIntervalId = setInterval(() => fetchAccountData(false), REFRESH_INTERVAL_MS);
      } else {
