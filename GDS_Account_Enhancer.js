// ==UserScript==
// @name         GDS 账户信息增强版 (v3.2.87-mod10 - 稳定布局优化版)
// @namespace    http://tampermonkey.net/
// @version      3.2.87.10
// @description  [v3.2.87-mod10]: 优化日志导出功能，防止在处理海量日志时导致浏览器短暂卡顿。
...
(async function() {
  ...
  const MAX_LOG_DB = 400000, MAX_FROZEN_LOG_DB = 2500;
  const MAX_LOG_MEM = 2000, MAX_FROZEN_LOG_MEM = 200;
  const MAX_BAL_HISTORY = 40;
  const PAYEE_OPTS = [ { name: '承兑KVB', payeeId: 110 }, { name: '承兑YES', payeeId: 804 }, { name: 'ABC代付', payeeId: 1162}, { name: 'NAMA2', payeeId: 2656}, { name: 'CRYSTAL IMPEX', payeeId: 2660}, { name: 'KortyaPay代付', payeeId: 2574}, { name: 'ISK', payeeId: 565} ];
  const TRANSFER_MODE_OPTS = [ { name: 'IMPS', transferMode: 1 }, { name: 'NEFT', transferMode: 2 }, { name: 'RTGS', transferMode: 3 }, ];
  const TRANSFER_PERCENT_OPTS = [ { name: '40%', value: 0.40 }, { name: '50%', value: 0.50 }, { name: '60%', value: 0.60 }, { name: '70%', value: 0.70 }, { name: '80%', value: 0.80 }, { name: '90%', value: 0.90 }, { name: '95%', value: 0.95 }, { name: '98%', value: 0.98 }, { name: '100%', value: 1.00 } ];
  ...
  function keepTabActive() {
    ...
    audio.src = 'data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA'; // 1-second silent wav
    audio.loop = true;
    const playPromise = audio.play();
    if (playPromise !== undefined) {
      playPromise.catch(error => {
        console.warn("GDS 脚本: 音频自动播放失败，后台刷新可能受限。请与页面交互一次（例如，点击任意位置）后，此功能可能会生效。", error);
      }).then(() => {
        console.log("GDS 脚本: 后台保活音频已启动，以尝试实现后台实时刷新。");
      });
    }
  }
  ...
  async function checkAndPerformAutoTransfers(specificAccId = null) {
    ...
    for (const accId of specificAccId ? [specificAccId] : Object.keys(accountDataCache)) {
        ...
        const payeeInfo = PAYEE_OPTS.find(p => p.payeeId === payload.payeeId);
        const payeeDisplay = payeeInfo ? `${esc(payeeInfo.name)} (ID: ${payload.payeeId})` : `PayeeID ${payload.payeeId}`;
        ...
        const commonLogMsg = `自动划转 ${fmtAmt(amtInCents / 100)} (随机金额: ${fmtCurrencyInt(finalTransferAmountYuan)} 元，原预计 ${fmtAmt(baseAmountFromSettings)}) 到 ${payeeDisplay} (模式: ${esc(modeName)})`;
  ...
  async function loadPersistedData() {
    ...
    try {
      ...
      const [cacheSetting, orderSetting, lastRefreshSetting, colVisSetting, sortConfSetting, logsVisSetting] = await Promise.all([
        ...
        loadSetting(KEYS.COLUMN_VIS, null),
  ...
  async function fetchAccountData(isInitialLoad = false) {
    ...
    try {
      ...
      if (!res.ok || res.rawJson?.code !== 1 || !Array.isArray(res.rawJson?.data?.list)) {
        ...
        if (Object.keys(accountDataCache).length === 0) tableContainer.innerHTML = `获取数据失败：${esc(errorMsg)}。请检查网络或Token。`; else renderTable();
        setTimeout(() => fetchAccountData(false), REFRESH_INTERVAL_MS); // 失败后 7 秒重试
        return;
      }
  ...
  // [优化] 日志导出功能，防止在处理海量日志时导致浏览器短暂卡顿
  async function exportLogs(e) {
      const exportButton = e.target;
      exportButton.disabled = true;
      showToast('正在准备并导出全部日志，请稍候...', e.clientX, e.clientY, 5000);

      setTimeout(async () => {
          try {
              const allOperationLogs = await dbHelper.getAll(STORES.OP_LOGS, null, 'prev');
              const allFrozenLogs = await dbHelper.getAll(STORES.FROZEN_LOGS, null, 'prev');

              console.log(`从数据库中获取了 ${allOperationLogs.length} 条操作日志和 ${allFrozenLogs.length} 条冻结日志用于导出。`);

              const searchTerms = searchInput.value.toLowerCase().trim().split(/\s+/).filter(k => k);
              let content = "GDS 账户信息增强版 - 全量日志导出\n";
              content += `导出时间: ${fmtDT(new Date())}\n`;
              ...
              const formatLogs = (logArray, title) => {
                  ...
                  if (matchedCount > 0) {
                      return `--- ${title} (共 ${logArray.length} 条记录, 匹配 ${matchedCount} 条) ---\n` + tempContent;
                  }
                  return '';
              };
              ...
              if (!opLogsContent && !frozenLogsContent) {
                  showToast('数据库中没有匹配当前搜索条件的日志可导出', e.clientX, e.clientY, 2000);
                  return;
              }
              ...
              a.href = url;
              a.download = `gds_full_logs_${fmtDT(new Date()).replace(/[:\s]/g, '_')}.txt`;
              document.body.appendChild(a);
              a.click();
              document.body.removeChild(a);
              URL.revokeObjectURL(url);
              showToast('全量日志已开始下载', e.clientX, e.clientY, 2000);
          } catch (err) {
              console.error("导出日志时发生错误:", err);
              showToast('导出日志失败，请查看控制台', e.clientX, e.clientY, 3000);
          } finally {
  ...
  async function initialize() {
    console.log(`GDS 账户信息增强版 (v3.2.87.10) 启动...`);
    ...
    if (!token && Object.keys(accountDataCache).length === 0) { tableContainer.innerHTML = '错误：未找到登录 Token 或 Token 刷新失败。请登录后刷新页面。'; }
  }
